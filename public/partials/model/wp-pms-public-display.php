...
function table_ordered_items($sql_result){
    ...
    foreach ($sql_result as $ordered_item){
        ...
        if(count($order_item_name_elements) != 1){
            array_push($new_arr, [$order_id, $order_item_name_elements[0], $order_item_name_elements[1], $order_item_name_elements[2],$img_array[0]]);
        }
    }
    return $new_arr;
}
...
/**
 * function responsible for creating the database and then inserting the new data
 */


/**
 * function responsible for changing the status of the product
 */
...
/**
 * function responsible for showing the animation of the status bas.
 */
    ...
    <header>
        ...
        <script type="text/javascript">
             function table_elements(index) {
                let chunked_content = <?php echo json_encode($chunked_table_items, true)?>
                let table_content = chunked_content[index];
                 let table_content_text = ""
                for(var i=0;i < table_content.length;i++){
                    table_content_text += table_content[i]+"\n";
                }
                /*8
                ;
                ...
                for(var i = 0;i < table_content.length; i++){
                 if(i === 0) {
                     table_content_text += "<tr><th scope='row'>" + i + "</th>";
                 }else if(i === table_content.length - 1){
                     table_content_text += "<td>table_content[i]</td></tr>";
                 }else {
                     table_content_text += "<td>table_content[i]</td>";
                 }
                }

                 */
                    document.getElementById("table_content").innerHTML = chunked_content[index.toString()];
             }
        </script>
    </header>
                ...
                <ul class="pagination">
                    <?php
                        for($i = 1; $i <= count($chunked_table_items);$i++){
                            $index = $i-1;
                            echo "<li class=\"page-item\" onclick=table_elements($index)><a class=\"page-link\">$i</a></li>";
                        }
                    ?>
                </ul>
            </nav>
